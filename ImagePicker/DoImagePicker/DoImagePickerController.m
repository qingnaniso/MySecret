...
- (void)viewDidLoad
{
    ...
    [_cvPhotoList registerNib:nib forCellWithReuseIdentifier:@"DoPhotoCell"]; //——cvPhotoList :CollectionView
...
//        NSLog(@"aGroups count = %lu",(unsigned long)[aGroups count]);
        NSInteger nIndex = 0;
        ...
        [_btSelectAlbum setTitle:@"手机相册" forState:UIControlStateNormal];
...
- (void)initBottomMenu
{
    ...
    else if (_nMaxCount <= 1)//可选的最大数小于1的话。。。
    {
        // hide ok button
        _btOK.hidden = YES;
        _ivLine1.hidden = YES;
    ...
    {
        _lbSelectCount.text = [NSString stringWithFormat:@"(0/%d)", (int)_nMaxCount]; // 0/?
        _lbSelectCount.textColor = DO_BOTTOM_TEXT_COLOR;
    }
}
...
{
//    NSLog(@"groupCount = %ld",(long)[ASSETHELPER getGroupCount]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *title;
    if ([d[@"name"] isEqualToString:@"Camera Roll"]) {
        title = @"手机相册";
        cell.lbAlbumName.text = title;
    } else if([d[@"name"] isEqualToString:@"My Photo Stream"]){
        title = @"相片流";
        cell.lbAlbumName.text   = title;
    } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *title = [ASSETHELPER getGroupInfo:indexPath.row][@"name"];
    if ([title isEqualToString:@"Camera Roll"]) {
        title = @"手机相册";
    }else if([title isEqualToString:@"My Photo Stream"]){
        title = @"相片流";
    }
    [_btSelectAlbum setTitle:title forState:UIControlStateNormal];
...
//        
        ...
        [UIView setAnimationDelay:0.1]; //延迟0.1秒执行下面的alpha动画
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView //函数很重要
{
    if (scrollView == _cvPhotoList)
    {
        [UIView animateWithDuration:0.2 animations:^(void) {
            //contentOffset 滑动的偏移量
//            NSLog(@"offset = %f", scrollView.contentOffset.y);
//            NSLog(@"frame.size.y = %f", scrollView.frame.size.height);
...
{
    //显然长按的手势操作在UIGestureRecognizerStateBegan时候应当计算选中的是哪一个cell
    if (_ivPreview != nil)
        return;
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan)
    {
        ...
        // check boundary of controls//排除其他控件长按的情况
        ...
        //可见控件中循环遍历 手势落在了哪个cell 这个cell就是当前cell
        for (UICollectionViewCell *cell in _cvPhotoList.visibleCells)
        {
            float fSX = cell.frame.origin.x;
            float fEX = cell.frame.origin.x + cell.frame.size.width;
...
- (void)showPhotosInGroup:(NSInteger)nIndex
{
    ...
    else if (_nMaxCount > 1)
    {
        ...
        _lbSelectCount.text = [NSString stringWithFormat:@"(0/%d)", (int)_nMaxCount]; // 0/4
    }
    ...
    [ASSETHELPER getPhotoListOfGroupByIndex:nIndex result:^(NSArray *aPhotos) {
        ...
        _cvPhotoList.alpha = 0.3;   // 0.0是完全透明的 1.0是完全不透明得
        [UIView animateWithDuration:0.2 animations:^(void) {
...
{
    CGPoint translation = [gestureRecognizer translationInView:self.view];//pan是拖拽，此函数在view中拖拽的距离

    if (gestureRecognizer.state == UIGestureRecognizerStateEnded)//一系列动作完成之后调用
    {
        [UIView animateWithDuration:0.2 animations:^(void) {
    ...
    {
		_ivPreview.center = CGPointMake(_ivPreview.center.x, _ivPreview.center.y + translation.y); //按纵坐标移动 跟随手指
		[gestureRecognizer setTranslation:CGPointMake(0, 0) inView:self.view];//移动值归零
        
        _vDimmed.alpha = 1 - ABS(_ivPreview.center.y - _vDimmed.center.y) / (self.view.frame.size.height / 2.0); //淡出
    }
}
